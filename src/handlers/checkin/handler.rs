...
use super::common::{
    get_existing_pending_ticket_id_for_ticket_in_id_bect, normalize_station_type,
...
pub(crate) async fn process_checkin(
    ...
    if account_id > 0 && !bect_skip_account_check() && apply_min_balance_check {
        ...
        if min_bal > 0 {
            ...
            let account_opt =
                tokio::task::spawn_blocking(move || get_account_for_charge(&pool_min, id_fetch))
                    .await
                    .unwrap_or(Ok(None))
                    .unwrap_or(None);
    ...
    match transport_service.save(&transport_stage).await {
        Ok(transport_trans_id) => {
            ...
            let verified_record_opt = match transport_service.get_by_id(transport_trans_id).await {
                Ok(Some(r)) => Some(r),
                Ok(None) => {
