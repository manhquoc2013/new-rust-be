...
pub(crate) async fn process_checkin(
    ...
    if lane_type == lane_type::OUT {
        tracing::debug!(request_id = fe_checkin.request_id, etag = %fe_checkin.etag, "[CHECKIN] OUT lane lookup ETDR");
        ...
        tracing::debug!(conn_id, request_id = fe_checkin.request_id, etag = %etag_query, elapsed_ms, step = "resolve_etdr_out", "[CHECKIN] CHECKIN step finished");
        ...
        match etdr_opt {
            Some(mut etdr) => {
                    ...
                    || etdr.checkin_datetime == 0
                {
                    ...
                    fe_resp.plate = plate;
                    tracing::debug!(request_id = fe_checkin.request_id, plate_source = plate_source, plate = %fe_resp.plate, "[CHECKIN] checkout resp plate (already checked out)");
                ...
                tracing::debug!(request_id = fe_checkin.request_id, etdr = ?etdr, "[CHECKIN] ETDR");
                ...
                tracing::debug!(conn_id, request_id = fe_checkin.request_id, etag = %etag_str, elapsed_ms, step = "get_toll_fee_list_context_and_subscription", "[CHECKIN] CHECKIN external call finished");
                ...
                let (trans_amount, rating_details) = {
                    ...
                    .await
                    {
                        ...
                        Err(e) => {
                            let elapsed_ms = step_calc.elapsed().as_millis() as u64;
                            tracing::debug!(conn_id, request_id = fe_checkin.request_id, elapsed_ms, step = "calculate_toll_fee", error = %e, "[CHECKIN] CHECKIN external call finished");
                                ...
                                .allow_zero_fee_when_no_price_or_route
                            {
                                tracing::warn!(
                                    ...
                                    error = %e,
                                    "[CHECKIN] toll fee bypass: using 0 when no price/route"
                                );
                                (0, vec![])
                            } else {
                                tracing::warn!(
                                    ...
                                    station_out = fe_checkin.station,
                                    error = %e,
                                    "[CHECKIN] toll fee error: no price config or cannot calculate"
                                );
                                tracing::error!(
                                    provider = "BECT",
                                    conn_id,
                ...
                if !bect_skip_account_check() {
                    if let Some(ref sub) = subscriber_info {
                        ...
                        if account_id <= 0 {
                            tracing::warn!(conn_id, request_id = fe_checkin.request_id, etag = %etag_query, account_id, "[CHECKIN] CHECKOUT account check: account not found or inactive");
                            error_status = bect::ACCOUNT_IS_NOT_ACTIVE;
                        }
                    } else {
                        tracing::warn!(conn_id, request_id = fe_checkin.request_id, etag = %etag_query, "[CHECKIN] CHECKOUT account check: subscriber not found");
                    ...
                    .await
                    {
                        Ok(Ok(id)) => {
                            tracing::debug!(
                                ...
                                transport_trans_id,
                                "[CHECKIN] ACCOUNT_TRANSACTION created"
                            );
                            Some(id)
                        }
                        Ok(Err(e)) => {
                            tracing::error!(conn_id, request_id = fe_checkin.request_id, transport_trans_id, error = %e, "[CHECKIN] CHECKOUT ACCOUNT_TRANSACTION insert failed");
                            None
                        }
                        Err(_) => None,
                    }
                    ...
                    .await
                {
                    Ok(true) => {
                        tracing::debug!(
                            request_id = fe_checkin.request_id,
                            "[CHECKIN] Stage checkout updated in DB"
                        );
                            ...
                            match crate::utils::epoch_to_datetime_utc(etdr.checkin_datetime) {
                                ...
                                None => {
                                    tracing::warn!(
                                        ...
                                        "[CHECKIN] Invalid checkin_datetime, using current time"
                                    );
                        ...
                        if !rating_details_for_db.is_empty() {
                                ...
                                .await
                            {
                                Ok(()) => {
                                    tracing::debug!(
                                        ...
                                        "[CHECKIN] TRANSPORT_TRANS_STAGE_TCD saved"
                                    );
                                    let boo_details =
                                ...
                                Err(e) => {
                                    tracing::error!(
                                        ...
                                        transport_trans_id,
                                        error = %e,
                                        "[CHECKIN] TRANSPORT_TRANS_STAGE_TCD save_tcd_list failed"
                                    );
                            ...
                            {
                                Ok(()) => {
                                    tracing::debug!(request_id = fe_checkin.request_id, transport_trans_id, "[CHECKIN] TRANSPORT_TRANS_STAGE_TCD saved (single summary row)");
                                }
                                Err(e) => {
                                    tracing::error!(conn_id, request_id = fe_checkin.request_id, transport_trans_id, error = %e, "[CHECKIN] CHECKOUT save TRANSPORT_TRANS_STAGE_TCD summary failed");
                    ...
                    Ok(false) => {
                        tracing::warn!(
                            ...
                            transport_trans_id,
                            etag = %fe_checkin.etag.trim(),
                            "[CHECKIN] CHECKOUT TRANSPORT_TRANSACTION_STAGE update returned false"
                        );
                        // Đảm bảo ETDR vẫn có dữ liệu mới nhất khi ghi DB lỗi
                        save_etdr(etdr.clone());
                    }
                    Err(e) => {
                        tracing::error!(conn_id, request_id = fe_checkin.request_id, transport_trans_id, etag = %fe_checkin.etag.trim(), error = %e, "[CHECKIN] CHECKOUT TRANSPORT_TRANSACTION_STAGE update failed");
                        save_etdr(etdr.clone());
                    }
                }
                ...
                if error_status != 0 {
                    ...
                    fe_resp.station = etdr.station_id;
                    fe_resp.lane = etdr.lane_id;
                    tracing::debug!(request_id = fe_checkin.request_id, plate_source = plate_source, plate = %fe_resp.plate, "[CHECKIN] checkout resp plate (error path)");
                    let reply_bytes = serialize_and_encrypt_response(&fe_resp, encryptor).await?;
                    return Ok((reply_bytes, fe_resp.status));
                }
                ...
                fe_resp.plate = plate;
                tracing::debug!(request_id = fe_checkin.request_id, plate_source = plate_source, plate = %fe_resp.plate, "[CHECKIN] checkout resp plate (success)");
                // Checkout: station/lane = entry (entry lane)
                fe_resp.station = etdr.station_id;
                fe_resp.lane = etdr.lane_id;
                fe_resp.ticket_type = etdr.ticket_type;
