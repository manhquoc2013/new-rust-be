...
docker tag rust-core-transaction:latest hoanvu/rust-core-transaction:latest

# Push to registry
docker push hoanvu/rust-core-transaction:latest
```
...
**IMPORTANT**: All resources are configured to use a dedicated namespace `rust-core-transaction` instead of the default namespace. This improves security by isolating resources and preventing conflicts.
...
kubectl get namespace rust-core-transaction
```
...
```yaml
image: hoanvu/rust-core-transaction:latest
```

If you need to use a different image, update `k8s/deployment.yaml`:
```yaml
image: your-registry/rust-core-transaction:latest
```
...
kubectl get pods -n rust-core-transaction -l app=rust-core-transaction

# Check logs (specify namespace)
kubectl logs -n rust-core-transaction -l app=rust-core-transaction -f

# Check service (specify namespace)
kubectl get svc -n rust-core-transaction rust-core-transaction-service

# Check all resources in namespace
kubectl get all -n rust-core-transaction
```
...
```bash
kubectl get pods -n rust-core-transaction -l app=rust-core-transaction -o wide
```
...
kubectl logs -n rust-core-transaction -l app=rust-core-transaction --tail=500 | grep -E "consumer leader lock|CHECKIN_HUB_INFO|Kafka consumer"
```
...
```bash
for p in $(kubectl get pods -n rust-core-transaction -l app=rust-core-transaction -o name); do
  echo "=== $p ==="
  kubectl logs -n rust-core-transaction $p --tail=200 | grep -E "consumer leader|BOO1|BOO2|STARTUP"
done
```
...
```bash
LEADER_POD="rust-core-transaction-xxxx-yyyy"   # thay bằng tên pod leader
kubectl delete pod -n rust-core-transaction $LEADER_POD
```
...
```bash
kubectl get pods -n rust-core-transaction -l app=rust-core-transaction
kubectl logs -n rust-core-transaction -l app=rust-core-transaction --tail=100 | grep "consumer leader lock acquired"
```
...
Service `rust-core-transaction-service` dùng `ClusterIP` với nhiều endpoint (mỗi pod một endpoint). Client kết nối qua Service sẽ được K8s load-balance tới các pod.
...
kubectl logs -n rust-core-transaction -l app=rust-core-transaction --prefix=true --tail=20
```
...
```bash
kubectl get pdb -n rust-core-transaction
kubectl describe pdb rust-core-transaction-pdb -n rust-core-transaction
```
...
|----------|------------------|
| Nhiều replica chạy | `kubectl get pods -n rust-core-transaction -l app=rust-core-transaction` |
...
| PDB | `kubectl get pdb -n rust-core-transaction` |
...
```bash
kubectl apply -f k8s/deployment.yaml -n rust-core-transaction
```
...
kubectl create secret generic rust-core-transaction-secrets \
...
```bash
# List secrets
kubectl get secrets -n rust-core-transaction

# View secret (values are base64 encoded)
kubectl get secret rust-core-transaction-secrets -n rust-core-transaction -o yaml

# Decode a specific key (example)
kubectl get secret rust-core-transaction-secrets -n rust-core-transaction -o jsonpath='{.data.pass}' | base64 -d
```
...
kubectl scale deployment rust-core-transaction --replicas=3 -n rust-core-transaction

# Or edit deployment
kubectl edit deployment rust-core-transaction -n rust-core-transaction
```
...
kubectl logs -n rust-core-transaction <pod-name> -f
```

### Describe Pod

```bash
kubectl describe pod <pod-name> -n rust-core-transaction
```

### Exec into Pod

```bash
kubectl exec -it <pod-name> -n rust-core-transaction -- /bin/bash
```
...
```bash
# Events in namespace
kubectl get events -n rust-core-transaction --sort-by=.metadata.creationTimestamp
...
kubectl get deployment -n rust-core-transaction

# Check pod status
kubectl get pods -n rust-core-transaction
```
...
- ✅ Dedicated namespace `rust-core-transaction` instead of `default`
...
- ✅ Dedicated ServiceAccount (`rust-core-transaction-sa`) instead of default
...
kubectl get namespace rust-core-transaction -o yaml | grep pod-security

# Check ServiceAccount (verify automountServiceAccountToken is false)
kubectl get serviceaccount rust-core-transaction-sa -n rust-core-transaction -o yaml | grep automountServiceAccountToken
# Should show: automountServiceAccountToken: false

# Check RBAC
kubectl get role,rolebinding -n rust-core-transaction

# Verify service account token is NOT mounted in pod
kubectl get pod <pod-name> -n rust-core-transaction -o yaml | grep -i "serviceAccount\|token"
...
kubectl exec -n rust-core-transaction <pod-name> -- ls -la /var/run/secrets/kubernetes.io/serviceaccount 2>&1
# Should fail: No such file or directory

# Check Pod Security Context (pod level)
kubectl get pod <pod-name> -n rust-core-transaction -o yaml | grep -A 30 "spec:" | grep -A 20 "securityContext"

# Check Container Security Context
kubectl get pod <pod-name> -n rust-core-transaction -o jsonpath='{.spec.containers[0].securityContext}'

# Check NetworkPolicy
kubectl get networkpolicy -n rust-core-transaction

# Check PodDisruptionBudget
kubectl get poddisruptionbudget -n rust-core-transaction

# Verify pod is running as non-root
kubectl exec -n rust-core-transaction <pod-name> -- id
# Should show: uid=1000(appuser) gid=1000(appuser)

# Verify read-only root filesystem
kubectl exec -n rust-core-transaction <pod-name> -- touch /test.txt
# Should fail with: touch: /test.txt: Read-only file system

# Verify capabilities are dropped
kubectl exec -n rust-core-transaction <pod-name> -- capsh --print
...
5. **Regular Security Audits**
   ...
   kubectl get all -n rust-core-transaction -o yaml > audit.yaml
   
   # Use tools like kube-score or kubeaudit
   kube-score score k8s/deployment.yaml
   
   # Verify service account token is not mounted
   kubectl get pod <pod-name> -n rust-core-transaction -o jsonpath='{.spec.automountServiceAccountToken}'
   # Should show: false
   ```
...
kubectl delete -f k8s/ -n rust-core-transaction

# Or delete namespace (this will delete all resources in it)
kubectl delete namespace rust-core-transaction
